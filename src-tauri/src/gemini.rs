...
// 识别两类：
//   1) 已解析的 opencode/codex/claude/gemini 原生二进制路径
//   2) bundle 里的 node 跑 gemini acp runner / gemini 包的 index.js --acp
...
pub fn cleanup_stale_runtime_orphans(app: &AppHandle) {
    ...
    // 故意用文件名而非完整路径匹配 —— app 升级 / dev→release 路径会变，但只要
    // 命中我们的 staged node + 我们的脚本 basename 就认它是自家孤儿。
    let runner_script_basename = "gemini-acp-runner.mjs";
    let gemini_entry_basename = "gemini/package/dist/index.js";
    ...
    for raw_line in String::from_utf8_lossy(&output.stdout).lines() {
        ...
        // node 跑 gemini 相关脚本的孤儿：我们的 staged node + 已知脚本 basename。
        // 不用完整脚本路径 —— dev/release/升级后路径会漂，basename 不变
        let hits_gemini_node = command.contains(&node_path)
            && (command.contains(runner_script_basename) || command.contains(gemini_entry_basename));

        if hits_native || hits_gemini_node {
...
pub fn shutdown_runtime_children(app: &AppHandle) {
    ...
    // 阻塞拿锁 —— 退出时必须清干净，try_lock 拿不到就静默漏是老 bug
    let (gemini_client, active_runs) = {
        let mut gemini = match state.gemini.lock() {
            Ok(guard) => guard,
    ...
    // 最后兜底：遍历我们进程的所有直系子进程（包括未进 state 的 gemini warmup、
    // ACP 初始化残留等），递归杀整棵子树。退出前杀掉，否则它们在我们死后会
    // 被 launchd 收养成 ppid==1 孤儿，而那时已没人能扫它们。
...
fn kill_all_direct_children() {
    ...
    // 换思路：遍历 WMIC 或 Get-CimInstance 找 ParentProcessId == self 的进程逐个 taskkill
    let current_pid = std::process::id();
    let script = format!(
